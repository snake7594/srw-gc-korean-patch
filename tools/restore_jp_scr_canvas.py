...
canvases: a translated label that is wider than the retail one bleeds into
the neighbouring value column, and one that is narrower leaves retail art
uncovered.

This tool rewrites the declared canvas of every translated ``SCR`` block
back to the Japanese retail value before any Korean is rendered.  The block
payload is resized to ``align32(32 + 2 * width * height)`` accordingly and
the outer absolute-offset table is recomputed, exactly as the atlas-growth
path in ``repack_preserve_indices.py`` already does.

Policy (v2): *every* translated SCR is restored to the Japanese canvas.
The earlier conservative exclusions (decorated canvases, labels whose
Korean ink would come out slightly smaller, labels the English patch
rebuilt into different artwork such as 891/897, and the small GX10
atlases) are gone:

* decorated canvases are supported because ``repack_direct_scr_atlas.py``
  now reconstructs the retail shelf art directly from the Japanese view,
* a slightly smaller Korean rendering on the retail canvas is preferred
  over overflowing the retail layout,
* 891 (読み方) and 897 (建物) are plain retail labels that the English
  patch replaced with larger legend artwork; the retail canvas is the one
  the in-game layout reserves space for,
* records of the small-atlas mapping (``--small-mapping``) are restored
  too, which ``repack_small_ui_atlases.py`` accepts as restored canvases.

SCR blocks whose header fields other than the canvas differ from the
Japanese retail block are treated as an error rather than skipped, so any
future drift stops the build loudly.
...
the downstream repack tools, so the copied entries only keep the
intermediate container self-consistent.
"""
...
def load_targets(
    large_mapping: Path, small_mapping: Path | None
) -> list[dict[str, object]]:
    """Return one row per translated SCR from both mapping files."""

    document = json.loads(large_mapping.read_text(encoding="utf-8"))
    if document.get("schema") != "srw-gc-ui-block518-ko-v1":
        raise ValueError(f"unsupported mapping schema: {large_mapping}")
    targets = [
        {
            "bitmap_block": 518,
    ...
    if small_mapping is not None:
        document = json.loads(small_mapping.read_text(encoding="utf-8"))
        if document.get("schema") != "srw-gc-ui-small-title-ko-v1":
            raise ValueError(f"unsupported mapping schema: {small_mapping}")
        for row in document["records"]:
            if str(row.get("action")) != "replace_with_korean":
                continue
            targets.append(
                {
                    "bitmap_block": int(row["bitmap_block"]),
                    "block_index": int(row["block_index"]),
                    "korean": str(row["korean_proposal"]).strip(),
                }
            )
    indices = [int(row["block_index"]) for row in targets]
    if len(indices) != len(set(indices)):
        raise ValueError("mappings share one or more SCR block indices")
    return sorted(targets, key=lambda row: int(row["block_index"]))
...
def main(argv: list[str] | None = None) -> int:
    ...
    protected = {source_path, japanese_path, large_mapping_path, small_mapping_path}
    ...
    targets = load_targets(large_mapping_path, small_mapping_path)
    ...
    for row in targets:
        ...
        if japanese.blocks[index][:4] != b"SCR\0":
            raise ValueError(f"Japanese block {index} is not an SCR")
        ...
        if block[12:32] != japanese.blocks[index][12:32]:
            raise ValueError(
                f"SCR {index} header fields other than the canvas differ from "
                "Japanese retail; refusing to restore blindly"
            )
    ...
    # Hard parity guarantee: after this pass no translated SCR may disagree
    # with the Japanese retail canvas.
    residual = [
        int(row["block_index"])
        for row in targets
        if struct.unpack_from(">II", verified.blocks[int(row["block_index"])], 4)
        != struct.unpack_from(">II", japanese.blocks[int(row["block_index"])], 4)
    ]
    if residual:
        raise RuntimeError(f"translated SCRs still off the Japanese canvas: {residual}")
    ...
    report = {
        "schema": "srw-gc-add00-jp-scr-canvas-restore-v2",
        "status": "pass",
        ...
        "translated_scr_count": len(targets),
        "restored_scr_count": len(restored),
        "skipped_scr_count": len(skipped),
        "translated_scr_dimension_mismatches_after": len(residual),
        ...
        json.dumps(
            {
                ...
                "translated_scr_dimension_mismatches_after": 0,
