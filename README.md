...
최신 배포본은 [v1.0.11 릴리스](https://github.com/snake7594/srw-gc-korean-patch/releases/tag/v1.0.11)에서 받을 수 있습니다.

**v1.0.1은 전투 화면의 동적 타일 번호를 손상할 수 있으므로 사용하지 마세요.**
**v1.0.3은 인물·로봇도감 설명문에서 글자 깨짐과 잘못된 메모리 읽기가 발생할 수 있어 철회했으므로 사용하지 마세요.**
**v1.0.4는 정신기 설명 23건이 다른 정신기에 연결되므로 v1.0.11로 교체하세요.**

## v1.0.11 주요 내용

전투 능력 라벨을 일본판 크기로 키우고, 오프닝 나레이션을 다시 번역한 릴리스입니다.

- 전투 화면 능력 라벨 23종(명중·치명타·원호·합체 공격 등)을 8×8 축소 표기에서 **일본판 크기(잉크 19~24px)**로 전환
  - 영문판 패치가 검증한 아틀라스 제자리 교체 방식을 사용자 승인 하에 적용, 전 라벨이 일본판 대비 ±1px
- 오프닝 스크롤 나레이션 34줄을 재번역 — 화면 폭을 넘어 잘리던 줄(최대 34자)을 전부 원문 길이 이하(≤24자)로, 용어(기계수 군단 등)·괄호(『』)·문체 통일
- 지형 적응 헤더를 일본판과 같은 **공 육 해 우** 1자 표기로 통일(空陸海宇 열 중심 ±0.5px 정렬)
- '제 화까지 완료'의 회차 숫자 공간을 일본판과 동일한 32px로 확보(숫자와 글자 겹침 방지)
...
출력 경로를 생략하면 원본 ISO와 같은 폴더에 `Super Robot Taisen GC_Korean_v1.0.11.iso`가 만들어집니다. 스크립트는 패치 전 원본과 패치 후 결과의 SHA-256을 모두 검사합니다.
